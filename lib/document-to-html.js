...
function DocumentConversion(options, comments) {
    ...
    function htmlPathForParagraph(element, messages) {
        ...
        if (style) {
            var style_clone = clone(style);
            for (var i in  style_clone.to._elements) {
                if (style_clone.to._elements[i].tagName == 'ol') {
                    if (element.numbering && element.numbering.format) {
                        style_clone.to._elements[i].attributes['type'] = listTypes[element.numbering.format];
                    }
                }
            }
            return style_clone.to;
        } else {
            if (element.styleId) {
